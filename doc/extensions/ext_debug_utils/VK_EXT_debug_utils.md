...
By using the [`ext_debug_utils`] extension, developers  **can**  obtain more
information.
...
immediately, then what is the point of this extension? **RESOLVED** : We needed a replacement for `[`ext_debug_report`]` because
...
# Version history
- Revision 1, 2017-09-14 (Mark Young and all listed Contributors)  - Initial draft, based on `[`ext_debug_report`]` and `[`ext_debug_marker`]` in addition to previous feedback supplied from various companies including Valve, Epic, and Oxide games. 
