...
-    If [`max_draw_count`] is greater than or equal to `1`, ([`stride`] × ([`max_draw_count`] - 1) +  [`offset`] +  sizeof([`DrawIndirectCommand`])) **must**  be less than or equal to the size of [`buffer`]
-    If the count stored in [`count_buffer`] is equal to `1`, ([`offset`] +  sizeof([`DrawIndirectCommand`])) **must**  be less than or equal to the size of [`buffer`]
-    If the count stored in [`count_buffer`] is greater than `1`, ([`stride`] × (`drawCount` - 1) +  [`offset`] +  sizeof([`DrawIndirectCommand`])) **must**  be less than or equal to the size of [`buffer`]
