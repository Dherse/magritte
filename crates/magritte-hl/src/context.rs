...
impl<V: VulkanApplication> Context<V> {
    pub fn new(mut application: V) -> Result<Self, ContextError> {
        ...
        let entry = Arc::new(application.load_entry().map_err(|e| ContextError::EntryLoadingFailed(e.to_string()))?);
    ...
    pub fn resize(&mut self, new_size: Extent2D) -> Result<(), VulkanResultCodes> {
        todo!();
