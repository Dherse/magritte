...
//!See [Unnormalized Texel Coordinate
//!Operations](https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/vkspec.html#textures-unnormalized-to-integer)
//! - Normalized coordinates are scaled using coord × (dim - 1)
//!rather than coord × dim, where dim is the size of one
//!dimension of the image.
//!See [normalized texel coordinate
//!transform](https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/vkspec.html#textures-normalized-to-unnormalized).
//! - Partial derivatives are scaled using coord × (dim - 1)
//!rather than coord × dim.
//!See [Scale Factor Operation](https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/vkspec.html#textures-scale-factor).
...
//!# Known issues & F.A.Q
//!0. What should this extension be named?**DISCUSSION**: While naming this extension, we chose the
//! most distinctive
...
//!1. Do we need a format feature flag so formats can advertise if they support
//! corner-sampling?**DISCUSSION**: Currently NVIDIA supports this for all 2D and 3D formats, but
