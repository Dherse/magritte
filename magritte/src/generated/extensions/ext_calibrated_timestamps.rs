...
impl std::fmt::Debug for TimeDomainEXT {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        struct Flags(TimeDomainEXT);
        impl std::fmt::Debug for Flags {
            #[allow(unused_assignments, unused_mut, unused_variables)]
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
                if self.0 == TimeDomainEXT::empty() {
                    f.write_str("empty")?;
                } else {
                    match self.0 {
                        TimeDomainEXT::DEVICE => f.write_str("DEVICE")?,
                        TimeDomainEXT::CLOCK_MONOTONIC => f.write_str("CLOCK_MONOTONIC")?,
                        TimeDomainEXT::CLOCK_MONOTONIC_RAW => f.write_str("CLOCK_MONOTONIC_RAW")?,
                        TimeDomainEXT::QUERY_PERFORMANCE_COUNTER => f.write_str("QUERY_PERFORMANCE_COUNTER")?,
        ...
        f.debug_tuple(stringify!(TimeDomainEXT)).field(&Flags(*self)).finish()
    }
}
...
impl<'lt> CalibratedTimestampInfoEXT<'lt> {
    ...
    pub fn make_static(mut self) -> CalibratedTimestampInfoEXT<'static> {
        unsafe {
    ...
    ///Sets the value of [`Self::time_domain`]
    pub fn with_time_domain(mut self, value: crate::extensions::ext_calibrated_timestamps::TimeDomainEXT) -> Self {
        self.time_domain = value;
        self
    }
}
