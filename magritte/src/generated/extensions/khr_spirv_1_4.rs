...
//!from high-level languages, rather than exposing new hardware functionality.SPIR-V 1.4
//! incorporates features that are also available separately as
//!extensions.
//!SPIR-V 1.4 shader modules do not need to enable those extensions with the
//!`OpExtension` opcode, since they are integral parts of SPIR-V 1.4.SPIR-V 1.4 introduces new
//! floating point execution mode capabilities, also
...
//!API changes. **RESOLVED** : Just expose SPIR-V 1.4.Most new SPIR-V versions introduce
//! optionally-required capabilities or have
...
//!versions? **RESOLVED** : Yes.Mixing SPIR-V versions 1.0-1.3 in the same pipeline has not been
//! disallowed,
...
//!module? **RESOLVED** : No, with caveats.The SPIR-V 1.4 module does not need to declare the
//! SPIR-V extensions, since
...
//!query can only be used if the extension is enabled.This applies to any SPIR-V version;
//! specifically for SPIR-V 1.4 this only
...
//!still optional in implementations that support [`VK_KHR_spirv_1_4`].A SPIR-V 1.4 module does not
//! need to enable `SPV_KHR_float_controls` in
...
//! * - Alexander Galazin, Arm  - David Neto, Google  - Jesse Hall, Google  - John Kessenich, Google
//!   - Neil Henning, AMD  - Tom Olson, Arm
...
pub const KHR_SPIRV_1_4_EXTENSION_NAME: &'static CStr = crate::cstr!("VK_KHR_spirv_1_4");
