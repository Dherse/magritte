...
use crate::vulkan1_0::{
    BaseInStructure, BaseOutStructure, Bool32, Buffer, CommandBuffer, Device, DeviceSize, QueryControlFlags, QueryPool,
    StructureType,
};
...
///   element of [`p_counter_buffers`] that is [`crate::utils::Handle::null`], transform feedback
///   will start capturing vertex data to byte zero in the corresponding bound transform feedback
///   buffer.
...
/// - For each buffer handle in the array, if it is not [`crate::utils::Handle::null`] it  **must**
...
///   [`crate::utils::Handle::null`] it  **must**  have been created with a `usage` value containing
///   `VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT`
        ...
        F: Fn(Device, &'static CStr) -> Option<extern "system" fn()>,
    {
        Self {
            cmd_bind_transform_feedback_buffers_ext: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdBindTransformFeedbackBuffersEXT")))
            },
            cmd_begin_transform_feedback_ext: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdBeginTransformFeedbackEXT")))
            },
            cmd_end_transform_feedback_ext: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdEndTransformFeedbackEXT")))
            },
            cmd_begin_query_indexed_ext: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdBeginQueryIndexedEXT")))
            },
            cmd_end_query_indexed_ext: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdEndQueryIndexedEXT")))
            },
            cmd_draw_indirect_byte_count_ext: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdDrawIndirectByteCountEXT")))
            },
        }
    }
