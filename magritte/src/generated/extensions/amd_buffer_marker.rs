...
use crate::{
    vulkan1_0::{Buffer, CommandBuffer, Device, DeviceSize, PipelineStageFlagBits},
    AsRaw, Unique,
};
use std::ffi::CStr;
...
impl CommandBuffer {
    ...
    ///   feature is not enabled, [`pipeline_stage`] **must**  not be `VK_PIPELINE_STAGE_NONE`
    /// - [`dst_offset`] **must**  be less than or equal to the size of [`dst_buffer`] minus `4`
    /// - [`dst_buffer`] **must**  have been created with `VK_BUFFER_USAGE_TRANSFER_DST_BIT` usage
    ///   flag
    ...
    pub fn load(
        ...
        Self {
            cmd_write_buffer_marker_amd: unsafe {
                std::mem::transmute(loader_fn(loader, crate::cstr!("vkCmdWriteBufferMarkerAMD").as_ptr()))
            },
        }
    }
