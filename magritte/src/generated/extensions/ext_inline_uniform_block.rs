...
//!both uniform buffers and inline uniform blocks.2) Is the descriptor array index and array size
//! expressed in terms of bytes
//!or dwords for inline uniform block descriptors? **RESOLVED** : In bytes, but both  **must**  be
//! a multiple of 4, similar to how push
...
//!uniform block bindings with must be contiguous in memory.3) What layout rules apply for uniform
//! blocks corresponding to inline
//!constants? **RESOLVED** : They use the same layout rules as uniform buffers.4) Do we need to add
//! non-uniform indexing features/properties as introduced
//!by [`VK_EXT_descriptor_indexing`] for inline uniform blocks? **RESOLVED** : No, because inline
//! uniform blocks are not allowed to be
//!“arrayed”.
...
//!level of support for update-after-bind inline uniform blocks.5) Is the
//! `descriptorBindingVariableDescriptorCount` feature introduced
//!by [`VK_EXT_descriptor_indexing`] supported for inline uniform blocks? **RESOLVED** : Yes, as
//! long as other inline uniform block specific limits are
...
//! * No known IP claims.
//! * - Daniel Rakos, AMD  - Jeff Bolz, NVIDIA  - Slawomir Grajewski, Intel  - Neil Henning,
//!   Codeplay
//!# Related
...
pub const EXT_INLINE_UNIFORM_BLOCK_EXTENSION_NAME: &'static CStr = crate::cstr!("VK_EXT_inline_uniform_block");
