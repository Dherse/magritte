...
/// The type of external synchronization of a field
...
pub enum ExternallySynced<'a> {
    ...
    /// Multiple values have to be externally synchronized
    Multiple(Vec<ExternallySynced<'a>>),

    /// A specific variable must be externally synchronized
    Variable(&'a str),

    /// All the values in an array must be synchronized
    All(Box<ExternallySynced<'a>>),

    /// A variable inside of a variable must be externally synchronized
    Resolve(Box<ExternallySynced<'a>>, Box<ExternallySynced<'a>>),

    /// All of the elements of an array must be externally synchronized
    ForEach(Box<ExternallySynced<'a>>, Box<ExternallySynced<'a>>),
}

impl<'a> ExternallySynced<'a> {
    /// Creates a new externally synchronized value from its external synchronization
    pub fn new<A: AsRef<str>>(value: Option<&'a A>) -> ExternallySynced<'a> {
        if let Some(input) = value.map(AsRef::as_ref) {
