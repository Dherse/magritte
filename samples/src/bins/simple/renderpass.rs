use log::info;
use magritte::{vulkan1_0::{VulkanResultCodes, AttachmentDescription, SampleCountFlagBits, AttachmentLoadOp, AttachmentStoreOp, ImageLayout, Format, AttachmentReference, SubpassDependency, SUBPASS_EXTERNAL, PipelineStageFlags, AccessFlags, RenderPass as VkRenderPass, SubpassDescription, PipelineBindPoint, RenderPassCreateInfo, Framebuffer, FramebufferCreateInfo}, Unique, SmallVec, AsRaw};
use magritte_samples::{vulkan::Vulkan, surface::Surface, depth::Depth};
...
impl RenderPass {
    ...
    fn create_framebuffers(surface: &Surface, depth: &Depth, renderpass: &Unique<VkRenderPass>) -> Result<SmallVec<Unique<Framebuffer>>, VulkanResultCodes> {
        surface.swapchain_image_views()
            ...
            .map(|view| {
                ...
                unsafe {
                    depth.image().device().create_framebuffer(
    ...
    fn create_renderpass(vulkan: &Vulkan, surface: &Surface) -> Result<Unique<VkRenderPass>, VulkanResultCodes> {
        ...
        unsafe {
            vulkan.device().create_render_pass(&renderpass_create_info, None).result()
        }
    }
