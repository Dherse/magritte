...
use magritte::{Extensions, vulkan1_0::{VulkanResultCodes, Semaphore, SemaphoreCreateInfo, BufferUsageFlags, ClearColorValue, ClearValue, ClearDepthStencilValue, RenderPassBeginInfo, Rect2D, Offset2D, PipelineStageFlags, SubpassContents, PipelineBindPoint, Viewport, IndexType}, SmallVec, Unique, AsRaw, extensions::khr_swapchain::PresentInfoKHR, window::create_surface};

use magritte_samples::{commands::Commands, queue::Queue, surface::Surface, vulkan::Vulkan, depth::Depth, buffer::Buffer};
use pipeline::Pipeline;
use renderpass::RenderPass;
use winit::{dpi::LogicalSize, event_loop::{EventLoop, ControlFlow}, window::{WindowBuilder, Window}, event::{Event, WindowEvent}};
        ...
        *control_flow = ControlFlow::Poll;
        ...
        match event {
            ...
            Event::MainEventsCleared => {
                ...
                if let Ok(swap_frame) = renderer.surface().acquire_next_image(&renderer.present_complete_semaphores()[frame]) {
                    let clear_values = [
                        ClearValue {
                                ...
                                cmd.cmd_set_viewport(
                                    ...
                                    &[
                                        Viewport {
                                            x: 0.0,
                                            y: 0.0,
                                            width: renderer.surface().extent().width as f32,
                                            height: renderer.surface().extent().height as f32,
                                            min_depth: 0.0,
                                            max_depth: 1.0,
                                        }
                                    ]
...
impl Drop for Renderer {
    fn drop(&mut self) {
        // As a caveat, we need to drop the command buffers first to enforce that they release all of their locks!
        // Magritte doesn't track usage in command buffers, therefore you will have to do it manually in some places.
        // The goal of Magritte is to decrease the necessity of these manual operations, but they cannot trivially all
        // be removed!
        unsafe {
...
impl Renderer {
    ...
    pub fn new(
        ...
        for _ in 0..surface.image_count() {
            present_complete_semaphores.push(unsafe {
                vulkan.device().create_semaphore(&semaphore_create_info, None).result()?
            });
        ...
        let pipeline = Pipeline::new(&vulkan, &renderpass)?;
    ...
    pub fn resize(&mut self, window: &Window) -> Result<(), VulkanResultCodes> {
        ...
        // We create a new renderpass
        self.renderpass = RenderPass::new(self.vulkan(), self.surface(), self.depth())?;
